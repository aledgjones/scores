...
const ManagePlaylistMember: FC<Props> = ({ playlistKey, uid, onClose }) => {
  const { user, mutate } = usePlaylistMember(playlistKey, uid);
  const initial = user?.name?.slice(0, 1).toLocaleUpperCase();
  const color = stringToColor(user?.email || "");

  const revoke = async () => {
    await revokePlaylistAccess(playlistKey, uid);
    onClose();
    await wait(200);
    mutate();
  };
  ...
  const updatePermissions = async (read: boolean, write: boolean) => {
    try {
      await updatePlaylistUserPermissions(playlistKey, user, read, write);
      mutate();
    } catch (error) {
...
export default BottomSheet(ManagePlaylistMember);
